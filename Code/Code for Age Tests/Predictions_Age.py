...
training_data = pd.read_csv('../Combined Tests/75_Age.csv')
...
# Load player data
player_data = pd.read_csv('/Combined Tests/Advanced_Stats.csv')

# Function to predict player's win shares 5 years down the line
def predict_future_win_shares(player_age, current_win_shares):
    # Predict the peak based on the polynomial regression model
    peak_prediction = poly_reg.predict(poly.transform([[player_age]]))[0]

    # Predict the win shares 5 years down the line
    future_age = player_age + 5
    future_poly = poly.transform(pd.DataFrame([[future_age]], columns=['Age']))
...
player_data['Predicted_WS_5_Years'] = player_data.apply(
    lambda row: predict_future_win_shares(row['Age_Adv'], row['WS']),
    axis=1
)
